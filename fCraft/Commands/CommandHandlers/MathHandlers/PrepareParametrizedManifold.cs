	...
	public static class PrepareParametrizedManifold {
		...
		public static void SetParamIteration(Player p, Command cmd) {
			...
			try {
				...
				p.Message("Iteration for " + strParam + " from " + from +
						  " to " + to + " with step " + step +
						  ". " + ((to - from)/step + 1) + " steps.");

				GetPlayerParametrizationParamsStorage(p)[VarNameToIdx(strParam[0])] = new double[] {
					from, to, step
				};
			} catch(Exception e) {
				p.Message("Error: " + e.Message);
		...
		private static string CoordsStorageName {
			get {
				return typeof(PrepareParametrizedManifold).Name + "Coords";
			}
		}

		private static string ParamsStorageName {
			get {
				return typeof(PrepareParametrizedManifold).Name + "Params";
			}
		}
