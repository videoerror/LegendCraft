	...
	internal class RealmHandler {
				...
				lock(WorldManager.SyncRoot) {
					...
					if(realm != null) {
						...
						if(!cmd.IsConfirmed &&
						   ...
						   File.Exists(targetFullFileName) &&
		...
		public static void RealmCreate(Player player, Command cmd, string themeName, string templateName) {
			...
			if(player.Name.Contains('.')) {
				// Support for email names.
				fileName = player.Name.Replace(".", "-");
			}
			...
			try {
				...
				Logger.Log(LogType.Error, "MapGenerator: Generation failed: {0}", ex);
					...
					switch(realm.BuildSecurity.Exclude(info)) {
						...
						case PermissionOverride.Allow:
							if(realm.BuildSecurity.Check(info)) {
								...
								player.Message("{0}&S is no longer allowed to build in {1}", info.ClassyName, realm.ClassyName);

								if(target != null) {
									target.Message("&WYou can no longer build in realm {0}&W(removed from whitelist by {1}&W).", realm.ClassyName, player.ClassyName);
								}
							}
					...
					if(rank == null) {
						...
						if(realm.BuildSecurity.MinRank == RankManager.LowestRank) {
							...
							Server.Message("{0}&S allowed only {1}+&S to build in realm {2}",
										   player.ClassyName,
										   realm.BuildSecurity.MinRank.ClassyName,
										   realm.ClassyName);
						}
